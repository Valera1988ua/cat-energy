 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (e => {
                if (functions_bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    if (document.querySelector(".header__content")) document.querySelector(".header__content").classList.toggle("open-menu");
                    document.querySelector("main").classList.toggle("open-menu");
                    document.querySelector("footer").classList.toggle("open-menu");
                }
            }));
        }
        ...
        let bodyLockToggle = function() {
            ...
            return document.documentElement.classList.contains("lock") ? functions_bodyUnlock(delay) : functions_bodyLock(delay);
        };
        ...
        let functions_bodyLock = function() {
            let delay = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 500;
            let body = document.querySelector("body");
            const wrapper = document.querySelector(".wrapper").offsetWidth;
            console.log(wrapper);
            if (functions_bodyLockStatus) {
                let lockPaddings = document.querySelectorAll("[data-lp]");
                for (const lockPadding of lockPaddings) lockPadding.style.paddingRight = window.innerWidth - wrapper + "px";
                body.style.paddingRight = window.innerWidth - wrapper + "px";
        ...
        const currentPageMenu = linksArr => {
        ...
        const hoverLinkMenu = () => {
            ...
            if (!navList) return;
            const navItem = navList.children;
            const navLinksArray = [ ...navItem ].map((item => item.querySelector(".menu__link")));
            const currentLink = currentPageMenu(navLinksArray);
            navLinksArray.forEach((link => {
                link.addEventListener("mouseenter", menuAction);
                link.addEventListener("mouseleave", menuAction);
            }));
            function menuAction(e) {
                const {type: typeEvent, target: targetElement} = e;
                if (typeEvent === "mouseenter") {
                    if (!targetElement.closest(".menu__list")) return;
                    if (!targetElement.classList.contains("_active-link")) {
                        navLinksArray.forEach((link => link.classList.remove("_active-link")));
                        targetElement.classList.add("_active-link");
                    }
                }
                if (typeEvent === "mouseleave") navLinksArray.forEach((link => {
                    if (!currentLink) return;
                    if (link.classList.contains("_active-link") && link.textContent === currentLink.textContent) return;
                    link.textContent === currentLink.textContent ? link.classList.add("_active-link") : link.classList.remove("_active-link");
                }));
            }
        };
        ...
        const map = () => {
            ...
            if (!mapBlock) return;
            const getMap = (entries, observer) => {
                const entrie = entries[0];
                if (!entrie.isIntersecting) return;
                const loadMapUrl = mapBlock.dataset.map;
                if (loadMapUrl) mapBlock.insertAdjacentHTML("beforeend", `<iframe src="${loadMapUrl}" width="100%" height="400" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>`);
                observer.unobserve(entrie.target);
            };
            const observer = new IntersectionObserver(getMap, {
                root: null,
                margin: 0,
                threshold: .1
            });
        ...
        const gotoblock_gotoBlock = function(targetBlock) {
            let noHeader = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
            let offsetTop = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 0;
            const targetBlockElement = document.querySelector(targetBlock);
            if (!targetBlockElement) functions_FLS(`[gotoBlock]: Такого блоку немає на сторінці: ${targetBlock}`);
            let headerItem = "";
            ...
            functions_FLS(`[gotoBlock]: Їдемо до ${targetBlock}`);
        };
        const modules_moduleRegistry = {};
        let formValidate = {
            getErrors(form) {
                let error = 0;
                let formRequiredItems = form.querySelectorAll("*[data-required]");
                if (formRequiredItems.length) formRequiredItems.forEach((formRequiredItem => {
                    if (formRequiredItem.offsetParent !== null && !formRequiredItem.disabled) error += this.validateInput(formRequiredItem);
                }));
                return error;
            },
            validateInput(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    this.addError(formRequiredItem);
                    error++;
                } else if (formRequiredItem.dataset.required === "name" && formRequiredItem.value.length < 2) {
                    this.addError(formRequiredItem);
                    error++;
                } else if (formRequiredItem.dataset.required === "weight" && formRequiredItem.value.length < 1) {
                    this.addError(formRequiredItem);
                    error++;
                } else if (formRequiredItem.dataset.required === "phone" && formRequiredItem.value.length === 10) {
                    this.addError(formRequiredItem);
                    error++;
            ...
            formClean(form) {
                ...
                if (checkboxes.length) for (const checkbox of checkboxes) checkbox.checked = false;
        ...
        const formSubmit = () => {
            ...
            async function openPopupWithDelay(popupDataset, delayTime) {
                await delay(delayTime);
                modules_moduleRegistry.popup.open(popupDataset);
            }
            async function formSent(form) {
                ...
                if (modules_moduleRegistry.popup) {
                    const popup = form.dataset.popupMessage;
                    await openPopupWithDelay(popup, 1);
                }
                await formValidate.formClean(form);
                formLogging(`Форму відправлено!`);
            }
            const formLogging = message => functions_FLS(`[Форми]: ${message}`);
        };
        const formFieldsInit = function() {
            ...
            if (options.autoHeight) {
                ...
                textareas.forEach((textarea => {
                    const startHeight = textarea.hasAttribute("data-autoheight-min") ? +textarea.dataset.autoheightMin : +textarea.offsetHeight;
                    const maxHeight = textarea.hasAttribute("data-autoheight-max") ? +textarea.dataset.autoheightMax : 1 / 0;
        ...
        const sliderAfterBefore = () => {
            const slider = document.querySelector(".slider-item__slider input");
            if (!slider) return;
            const sliderSpan = document.querySelector(".slider-item__drag-line span");
            const img = document.querySelector(".slider-item__after");
            slider.addEventListener("input", sliderAction);
            function sliderAction(e) {
                const sliderValue = e.target.value;
                sliderSpan.style.left = `${sliderValue}%`;
        ...
        class Popup {
            ...
            open(selectorValue) {
                if (functions_bodyLockStatus) {
                    ...
                    if (!this.targetOpen.element) {
                        this.popupLogging(`Такого попапу немає. Перевірте коректність введення. `);
                        return;
                    }
                    if (this.youTubeCode) {
            ...
            close(selectorValue) {
                ...
                if (!this.isOpen || !functions_bodyLockStatus) return;
        ...
        modules_moduleRegistry.popup = new Popup({});
        class DynamicAdapt {
            constructor(type) {
                this.type = type;
            }
            init() {
                ...
                this.mediaQueries.forEach((media => {
                    ...
                    const оbjectsFilter = this.objects.filter((_ref2 => {
        ...
        da.init();
        const productsBlock = document.querySelector(".catalog-products");
        const dataFile = "data/products.json";
        let more = false;
        window.addEventListener("load", (() => {
            if (productsBlock) getProducts(dataFile);
        }));
        const getProducts = async dataFile => {
            try {
                ...
                if (response.ok) {
                    if (more) {
                        displayProducts([ ...dataProducts.products.slice(7, dataProducts.products.length) ]);
                        return;
                    }
                    displayProducts([ ...dataProducts.products.slice(0, 7) ]);
                } else {
                    const div = document.createElement("div");
                    div.classList.add("error");
                    div.textContent = "Ощибка загрузки";
        ...
        const displayProducts = products => {
            products.forEach((product => {
                const {name, description, weight, taste, price, image} = product;
                const html = `\n               <article class="catalog-products__item _hide-product">\n                   <div style="transform:scale(${weight === "1000 г" ? "1.2" : "1"});" class="catalog-products__image">\n                        <img src="img/catalog/${image}" alt="${description}">\n                     </div>\n                  <h3 class="catalog-products__title">${name}<span>${weight}</span></h3>\n                     <ul class="catalog-products__list">\n                        <li class="catalog-products__list-item">Масса<span>${weight}</span></li>\n                        <li class="catalog-products__list-item">Вкус<span>${taste}</span></li>\n                        <li class="catalog-products__list-item">Цена<span>${price}</span></li>\n                     </ul>\n                     <a href="#" class="catalog-products__link link-button">заказать</a>\n                  </article>\n               `;
                productsBlock.insertAdjacentHTML("beforeend", html);
            }));
            const catalogItem = document.querySelectorAll(".catalog-products__item");
            if (catalogItem.length) catalogItem.forEach((item => {
                setTimeout((() => {
                    item.classList.remove("_hide-product");
                }), 500);
            }));
            if (more) return;
            const moreHtml = `\n                  <article class="catalog-products__item catalog-products__item_more">\n                     <div class="catalog-products__image">\n                        <img src="img/catalog/plus.svg" alt="Иконка">\n                     </div>\n                     <h3 class="catalog-products__title">Показать еще<span>100500 товаров</span></h3>\n                     <div class="catalog-products__text text">На самом деле вкусов гораздо больше!</div>\n                     <button type="button" class="catalog-products__more-button link-button">показать все</button>\n                  </article>\n               `;
            productsBlock.insertAdjacentHTML("beforeend", moreHtml);
            showMoreProduct();
        };
        const showMoreProduct = () => {
            const moreButton = document.querySelector(".catalog-products__more-button");
            if (!moreButton) return;
            moreButton.addEventListener("click", showProduct);
            function showProduct(e) {
                e.preventDefault();
                const {target: targetElement} = e;
                if (!targetElement.classList.contains("_hide")) {
                    const parent = targetElement.parentNode;
                    const image = parent.querySelector(".catalog-products__image img");
                    image.classList.add("_loading");
                    setTimeout((async () => {
                        more = true;
                        await getProducts(dataFile);
                        targetElement.parentNode.classList.add("_hide");
                    }), 1e3);
                }
            }
        };
